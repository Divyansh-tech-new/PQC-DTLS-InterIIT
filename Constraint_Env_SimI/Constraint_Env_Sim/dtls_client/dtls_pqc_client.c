...
int main(int argc, char* argv[])
{
    ...
    printf("[Client] Loading client certificate from %s...\n", CLIENT_CERT_FILE);
    ret = wolfSSL_CTX_use_certificate_file(ctx, CLIENT_CERT_FILE, WOLFSSL_FILETYPE_ASN1);
    if (ret != 1) {
        fprintf(stderr, "[Client] Failed to load client cert (ret=%d)\n", ret);
        goto exit;
    }
    printf("[Client] ✓ Client certificate loaded\n");

    printf("[Client] Loading client key from %s...\n", CLIENT_KEY_FILE);
    ret = wolfSSL_CTX_use_PrivateKey_file(ctx, CLIENT_KEY_FILE, WOLFSSL_FILETYPE_ASN1);
    if (ret != 1) {
        fprintf(stderr, "[Client] Failed to load client key (ret=%d)\n", ret);
        goto exit;
    }
    printf("[Client] ✓ Client private key loaded\n");

    /* Load CA certificate for server verification */
    printf("[Client] Loading CA certificate from %s...\n", CA_CERT_FILE);
    ret = wolfSSL_CTX_load_verify_locations(ctx, CA_CERT_FILE, NULL);
    if (ret != 1) {
        fprintf(stderr, "[Client] Failed to load CA cert (ret=%d)\n", ret);
        goto exit;
    }
    printf("[Client] ✓ CA certificate loaded\n");

    /* Enable peer verification */
    wolfSSL_CTX_set_verify(ctx, WOLFSSL_VERIFY_PEER, verify_callback);
    printf("[Client] ✓ Certificate-based mutual authentication configured\n");
