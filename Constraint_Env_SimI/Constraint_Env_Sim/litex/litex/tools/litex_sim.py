...
class SimSoC(SoCCore):
    def __init__(self,
        ...
        **kwargs):
        ...
        # Parameters -------------------------------------------------------------------------------
        sys_clk_freq = int(10e6)  # Changed from 1e6 to 10e6 for faster execution
...
def main():
    ...
    soc_kwargs.pop("with_uartbone", None)
    sys_clk_freq = int(10e6)  # Changed from 1e6 to 10e6 for faster execution
    ...
    # UART.
    print(f"DEBUG: uart_name is {soc_kwargs['uart_name']}")
    if soc_kwargs["uart_name"] == "serial":
        print("DEBUG: Setting up serial modules")
        # Inside the SoC we use the "sim" UART,
        # and connect it to the external "serial" endpoint.
        soc_kwargs["uart_name"] = "sim"

        # Console (BIOS prints to your terminal/log).
        # sim_config.add_module("serial2console", "serial")

        # TCP UART bridge (LiteX <-> external tools).
