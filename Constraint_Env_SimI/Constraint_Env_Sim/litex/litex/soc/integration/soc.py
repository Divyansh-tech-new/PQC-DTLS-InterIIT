...
class SoCController(LiteXModule):
    def __init__(self, with_reset=True, with_scratch=True, with_errors=True):
        if with_reset:
            self._reset = CSRStorage(name="reset", fields=[
        ...
        if with_scratch:
            self._scratch = CSRStorage(32, name="scratch", reset=0x12345678, description="""
        ...
        if with_errors:
            self._bus_errors = CSRStatus(32, name="bus_errors", description="Total number of Wishbone bus errors (timeouts) since start.")
