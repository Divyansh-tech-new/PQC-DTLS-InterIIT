...
echo "  Waiting for simulation to open port $LITEX_TCP_PORT..."

# Smart wait: Poll until port 1234 is available (max 120 seconds)
MAX_WAIT=120
WAITED=0
while ! ss -tlnp 2>/dev/null | grep -q ":$LITEX_TCP_PORT " && [ $WAITED -lt $MAX_WAIT ]; do
    sleep 2
    WAITED=$((WAITED + 2))
    if [ $((WAITED % 10)) -eq 0 ]; then
        echo "    [$WAITED seconds] Still waiting for simulation port..."
    fi
done

if ss -tlnp 2>/dev/null | grep -q ":$LITEX_TCP_PORT "; then
    echo "  ✓ Simulation port $LITEX_TCP_PORT is ready! (waited ${WAITED}s)"
else
    echo "  ✗ ERROR: Simulation failed to open port $LITEX_TCP_PORT after ${MAX_WAIT}s"
    echo "  Check logs/litex_sim.log for errors"
    exit 1
fi
